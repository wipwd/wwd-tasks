...
export class WeeklyReportComponent implements AfterViewInit, OnInit {
  ...
  public downloadAsRAG(): void {
    ...
    const week_start: number = start.week();
    const week_end: number = end.week();

    const str_start: string =
      `${start.year()}-${start.month() + 1}-${start.date()}`
    const str_end: string =
      `${end.year()}-${end.month() + 1}-${end.date()}`;
    let str_week: string = `${week_start}`
    if (week_start !== week_end) {
      str_week = `${week_start}-${week_end}`;
    }

    const nowstr: string = moment.utc().toString();

    const reportA: Report = new Report();
    reportA.setHeader(
      `# Work Report - week ${str_week} (from ${str_start} to ${str_end})`
    );
    reportA.setFooter(
    ...
    anchor.download = `weekly-report-W${str_week}.txt`;
    anchor.href = url;
    anchor.click();
  }
}
